...
public class BayesianAgent
    ...
    public static class PitfallBayesianNetwork
        ...
        /**
         * TODO: please replace this code. The code here will pick a **random** frontier
         * square to explore next,
         * which may be a pit! You should do the following steps:
         * 1) for each frontier square X, calculate the query Pr[Pit_X = true |
         * evidence]
         ...
         * If you calculate Pr[Pit_X = true && evidence] for every X, you can convert
         * the values into
         * probabilities by adding up all Pr[Pit_X = true && evidence] values and
         * dividing each
         * Pr[Pit_X = true && evidence] value by the sum.
         *
         * 2) pick the pit that is the least likely to have a pit in it to explore next!
         *
         * As an aside here, you can certainly choose to calculate Pr[Pit_X = false |
         * evidence] values
        ...
        public Coordinate getNextCoordinateToExplore() {
            ...
            Set<Coordinate> frontier = this.getFrontierPitCoordinates();
            Map<Coordinate, Boolean> breezes = this.getKnownBreezeCoordinates();
            Map<Coordinate, Float> probabilities = new HashMap<>();
            // System.out.println("frontier: " + frontier);
            // System.out.println("breezes: " + breezes);

            float sumProb = 0f;
            // Loop through frontiers
            for (Coordinate x : frontier) {
                // check if x is adjacent to any breezes and count the number of adjacent
                // breezes
                int count = 0;
                for (Coordinate y : getAdjacent(x)) {
                    if (breezes.containsKey(y) && breezes.get(y) == true) {
                        count++;
                    }
                }
                float prob = (float) (count * getPitProb());
                sumProb += prob;
                probabilities.put(x, prob);
            }

            if (sumProb > 0f) {
                for (Map.Entry<Coordinate, Float> entry : probabilities.entrySet()) {
                    probabilities.replace(entry.getKey(), entry.getValue() / sumProb);
                }
            }

            System.out.println("probabilities: " + probabilities);

            Set<Coordinate> lowest = new HashSet<>();
            float lowestProb = 100;

            // Get the lowest existing probability
            for (Map.Entry<Coordinate, Float> entry : probabilities.entrySet()) {
            ...
            // Get all coords with the lowest probability
            for (Map.Entry<Coordinate, Float> entry : probabilities.entrySet()) {
                if (entry.getValue() == lowestProb) {
            ...
            // Get a random coordinate from the ones with the lowest probability to break
            // ties
            toExplore = getRandom(lowest);
            System.out.println("toExplore: " + toExplore);

            return toExplore;
        }

        private Coordinate getRandom(Set<Coordinate> lowests) {
            if (lowests.size() == 0) {
                return null;
            }

            Coordinate[] array = lowests.toArray(new Coordinate[lowests.size()]);
            Random rand = new Random();
            Coordinate randomElement = array[rand.nextInt(array.length)];

            return randomElement;
        }

        private Set<Coordinate> getAdjacent(Coordinate frontier) {
            int xSrc = frontier.getXCoordinate();
            int ySrc = frontier.getYCoordinate();

            Coordinate left = new Coordinate(xSrc - 1, ySrc);
            Coordinate right = new Coordinate(xSrc + 1, ySrc);
            Coordinate up = new Coordinate(xSrc, ySrc + 1);
            Coordinate down = new Coordinate(xSrc, ySrc - 1);
        ...
        private boolean isAdjcent(Coordinate frontier, Coordinate dst) {
            int xSrc = frontier.getXCoordinate();
            int ySrc = frontier.getYCoordinate();
            int xdst = dst.getXCoordinate();
            int ydst = dst.getYCoordinate();

            // check left and right of frontier
            if ((xSrc == xdst + 1 || xSrc == xdst - 1) && ySrc == ydst) {
                return true;
            }

            // check up and down of frontier
            if ((ySrc == ydst + 1 || ySrc == ydst - 1) && xSrc == xdst) {
                return true;
            }
    ...
    public Map<Integer, Action> initialStep(StateView state,
        ...
        for (Integer unitID : state.getUnitIds(enemyPlayerNumber)) {
            if (!state.getUnit(unitID).getTemplateView().getName().toLowerCase().equals("hiddensquare")) {
                System.err
                        .println("ERROR [BayesianAgent.initialStep]: Enemy should start off with HiddenSquare units!");
                System.exit(-1);
            }
        ...
        initialActions.put(
                ...
                Action.createPrimitiveAttack(
                        this.getMyUnitID(),
                        state.unitAt(this.getSrcCoordinate().getXCoordinate(),
    ...
    public boolean isFrontierCoordiante(Coordinate src,
        ...
        for (int dir[] : dirs) {
            ...
            if (x >= 1 && x <= state.getXExtent() - 2 &&
                    ...
                    (!state.isUnitAt(x, y) ||
                            !state.getUnit(state.unitAt(x, y)).getTemplateView().getName().toLowerCase()
                                    .equals("hiddensquare"))) {
                return true;
            }
        }
    ...
    public Map<Integer, Action> middleStep(StateView state,
        ...
        if (Synchronizer.isMyTurn(this.getPlayerNumber(), state)) {
            ...
            // we need to check that the unit at that coordinate is a hidden square (not
            // allowed to attack other units)
